# How to provision a new environment for RailsStarter
...
* Enter domains to secure (for example): `railsstarter.com`
* Enter webroot path: `/home/deployer/apps/RailsStarter/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/railsstarter.com/fullchain.pem /etc/ssl/RailsStarter.crt
ln -s /etc/letsencrypt/live/railsstarter.com/privkey.pem /etc/ssl/RailsStarter.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/RailsStarter/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
18 7,19 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/RailsStarter/shared/public/
```
